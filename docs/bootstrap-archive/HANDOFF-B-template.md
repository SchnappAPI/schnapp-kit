...
You are a Claude Code session on Schnapps-MBP. The repo `schnappapi/schnapp-kit` was just created (at `{{KIT_REPO_CREATED_AT_UTC}}`, default branch `{{DEFAULT_BRANCH}}`, clone URL `{{KIT_REPO_URL}}`). Your job is to build out v1 of that repo, then clean up the bootstrap files that brought us here.
...
- schnapp-kit URL: `{{KIT_REPO_HTTPS}}`.
